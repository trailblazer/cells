  ...
  context "Invoking render" do
    ...
    should "raise an error for a non-existent template" do
      ...
      if Cells.rails3_0?
        ...
        assert_includes e.message, ":locale=>[:en]"
        assert_includes e.message, ":handlers=>[:erb, :builder, :haml]"
        assert_includes e.message, ":formats=>[:html, :text, :js, :css, :ics, :csv, :png, :jpeg, :gif, :bmp, :tiff, :mpeg, :xml, :rss, :atom, :yaml, :multipart_form, :url_encoded_form, :json, :pdf, :zip]"
      end
    end
